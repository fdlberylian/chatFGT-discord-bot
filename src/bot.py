...
async def send_message(message, user_message):
    ...
    try:
        ...
        if chat_model == "OFFICIAL":
            response = (
                f"{response}{await responses.official_handle_response(user_message)}"
            )
        elif chat_model == "UNOFFICIAL":
            response = (
                f"{response}{await responses.unofficial_handle_response(user_message)}"
            )
        char_limit = 1900
...
def run_discord_bot():
    ...
    async def on_message(message):
        ...
        if isReplyAll == "True" and message.channel.id == int(
            os.getenv("REPLYING_ALL_DISCORD_CHANNEL_ID")
        ):
